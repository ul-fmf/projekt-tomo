...
assert set(UPLOAD_STATUS_ICON) == set(US), f"Mismatched keys: {set(UPLOAD_STATUS_ICON)} vs. {set(US)}"


UPLOAD_STATUS_MEANINGS: Dict[int, 'StrOrPromise'] = {
	US.waiting: _("The submission is waiting in the evaluation queue for a free tester."),
...
assert set(UPLOAD_STATUS_MEANINGS) == set(US), f"Mismatched keys: {set(UPLOAD_STATUS_MEANINGS)} vs. {set(US)}"
...
assert set(JAILRUN_STATUS_ICON) == set(JRS), f"Mismatched keys: {set(JAILRUN_STATUS_ICON)} vs. {set(JRS)}"
...
JAILRUN_STATUS_MEANINGS = {
	...
	JRS.OUT: _("The program produced too much output (stdout or stderr), and was terminated. The usual limit is 100 MB."),
	JRS.THR: _("The program spawned too many threads or processes, and was terminated. The usual limit is 1 thread."),
	JRS.SYS: _(
	...
	JRS.WA: _("The program finished successfully, but the output produced was not entirely correct."),
	JRS.PE: _(
...
assert set(JAILRUN_STATUS_MEANINGS) == set(JRS), f"Mismatched keys: {set(JAILRUN_STATUS_MEANINGS)} vs. {set(JRS)}"
...
class SubtaskDetails:
	...
	def __post_init__(self):
		self.points = self.max_points if self.status == settings.JAILRUN_STATUS.OK else 0
...
def maybe_split_into_subtasks(task: 'Task', test_cases: List['TestCase']) -> Optional[List[SubtaskDetails]]:
