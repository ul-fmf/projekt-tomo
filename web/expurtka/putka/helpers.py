...
def csz(txt):
	...
	for slo, ascii in zip(u'čšžćđČŠŽĆĐ', 'cszcdCSZCD'):
...
def call_with_async_pipe(func: Callable[..., None], *args, **kwargs) -> IO[bytes]:
	...
	async def _thunk():
		asyncio.ensure_future(database_sync_to_async(_wrapped_func, thread_sensitive=False)())
...
class StructuredJsonResponse(JsonResponse):
	...
	def __init__(self, *, action, payload, **kwargs):
		...
		super().__init__({'action': action, 'payload': payload}, JSONProxyEncoder, **kwargs)

	@staticmethod
	def refresh():
		return StructuredJsonResponse(action=StructuredJsonResponse.REFRESH, payload=None)

	@staticmethod
	def redirect(url):
		return StructuredJsonResponse(action=StructuredJsonResponse.REDIRECT, payload=url)

	@staticmethod
	def success():
		return StructuredJsonResponse(action=StructuredJsonResponse.SUCCESS, payload=None)

	@staticmethod
	def display(content):
		return StructuredJsonResponse(action=StructuredJsonResponse.DISPLAY, payload=content)
...
class JsonResponseMixin:
	...
	def render_to_response(self: TemplateViewClassProtocol, context, **response_kwargs) -> HttpResponse:
		response = super().render_to_response(context, **response_kwargs)  # type:ignore[safe-super]
...
class LimitedTemporaryFileUploadHandler(TemporaryFileUploadHandler):
	...
	def new_file(self, field_name, file_name, content_type, content_length, charset=None, content_type_extra=None):
		...
		self._size_so_far = 0
		return super().new_file(field_name, file_name, content_type, content_length, charset=charset, content_type_extra=content_type_extra)
