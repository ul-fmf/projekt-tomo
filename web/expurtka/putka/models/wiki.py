...
class ContentQuerySet(models.QuerySet):
    def lang_pref_order(self, pref_lang: str) -> "ContentQuerySet":
        return self.alias(
            lang_pref=Case(
                When(lang=pref_lang, then=Value(1)),
                When(lang="en", then=Value(2)),
                default=3,
            )
...
class Content(models.Model):
    ...
    def save(self, update_fields=None, **kwargs):
        if "editor" not in kwargs:
            raise ValueError(
                "Argument `editor` missing when saving a Content instance."
            )
        self.version += 1
    ...
    def check_headings(self) -> List["StrOrPromise"]:
        ...
        for h in set(headings_done) - set(headings_all):
            warnings.append(
                _(
                    'Unknown section heading: "%(heading)s". Try to use only these: %(options)s'
                )
                % {"heading": h, "options": ", ".join(headings_all)}
            )
