...
def thread_content_types() -> list[int]:
    ...
    return list(
        map(
            lambda c: ContentType.objects.get_for_model(c).id,
            (GeneralThread, TaskThread, ContestThread),
        )
    )
...
class Thread(models.Model):
    ...
    thread_type = models.SlugField(
        max_length=1, choices=settings.THREAD_TYPES.choices, editable=False
    )
    ...
    def concrete(self) -> "Thread":
        ...
        if self.thread_type == settings.THREAD_TYPES.general:
            ...
            raise TypeError(
                f"Thread {self.pk} is not convertible to a concrete child class."
            )
...
class GeneralThread(Thread):
    ...
    url = models.SlugField(
        unique=True, verbose_name=_("URL"), validators=[PUTKA_SLUG_VALIDATOR]
    )
    forum = models.ForeignKey(
        Forum, related_name="threads", on_delete=models.CASCADE, verbose_name=_("Forum")
    )
    closed = models.BooleanField(
        default=False,
        verbose_name=_("Closed"),
        help_text=_("Closed threads do not allow adding new posts."),
    )
...
class TaskThread(Thread):
    ...
    task = models.OneToOneField(
        "expurtka.Task", related_name="thread", on_delete=models.CASCADE
    )
...
class ContestThread(Thread):
    ...
    contest = models.OneToOneField(
        Contest, related_name="thread", on_delete=models.CASCADE
    )
    ...
    def create_alerts(self):
        ...
        for u in listeners:
            if u == self.last_editor:
                continue
            alerts.append(
                Alert.for_thread_activity(user=u, thread=self.thread.concrete())
            )
