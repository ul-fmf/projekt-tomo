...
class Tag(models.Model):
    name = models.SlugField(
        ...
        blank=False,
        null=False,
        validators=[PUTKA_SLUG_VALIDATOR],
        help_text=_(
            "Tag name as displayed to the user. Separated with dashes and usually not capitalized. Currently not translatable."
        ),
    )
    description = models.TextField(
        ...
        null=False,
        help_text=_(
            "Describe which type of tasks this tag represents. You can specify what tasks do and what tasks don't belong under this tag."
        ),
    )
...
class TaskQuerySet(ContentQuerySetMixin, models.QuerySet):
    ...
    def with_active_contests_info(self) -> "TaskQuerySet":
        return self.annotate(
            in_active_contests=Exists(
                Contest.objects.ongoing().filter(set__tasks=OuterRef("pk"))
            )
        )

    def with_is_solved(self, user) -> "TaskQuerySet":
        return self.annotate(
            is_solved=Exists(user.upload_set.solved().filter(task_id=OuterRef("pk")))
        )

    def with_success_ratio(self) -> "TaskQuerySet":
        return (
            self.annotate(submitter_count=Count("upload__user", distinct=True))
            .annotate(
                submitter_count_ok=Count(
                    "upload__user", filter=self.TASKS_SOLVED_Q, distinct=True
                )
            )
            .annotate(
                submitter_count_float=Cast("submitter_count", output_field=FloatField())
            )
            .annotate(
                submitter_count_ok_float=Cast(
                    "submitter_count_ok", output_field=FloatField()
                )
            )
            .annotate(
                success_ratio=Case(
    ...
    class Meta:
        db_table = "ui_core_tasks_taskqueryset"
...
_SUBTASKS_REGEX = re.compile(
    r"(?:SUBTASKS\s*=|putka\.agg\.subtasked\()\s*\[\s*((?:\d+\s*,?\s*)+)]"
)
...
class Task(ContentManagementMixin, models.Model):
    ...
    url = models.SlugField(
        ...
        validators=[PUTKA_SLUG_VALIDATOR],
        help_text=_(
            "Preferably contains letters, numbers and dashes, but can be any slug. It must be globally unique."
        ),
    )
    author = models.CharField(
        ...
        help_text=_(
            'The "Author (Contest)" syntax may be used to specify the task source or the original contest it appeared in.'
        ),
    )
    evaluation_type = models.PositiveSmallIntegerField(
        choices=TASK_EVALUATION_TYPES.choices,
    ...
    difficulty = models.PositiveSmallIntegerField(
        ...
        help_text=_("Estimated task difficulty as intended by the author."),
    )
    tags = models.ManyToManyField(
        ...
        verbose_name=_("Tags"),
        help_text=_(
            "Add all applicable tags. Always try to add at least one tag. You can create a new one, if really needed."
        ),
    )
    testscript = models.TextField(null=False)
    parents = models.ManyToManyField(
        to="expurtka.Set", through="expurtka.TaskLink", related_name="tasks"
    )
    ...
    def difficulty_description(self) -> Optional[str]:
        return (
            settings.TASK_DIFFICULTY(self.difficulty).label if self.difficulty else None
        )

    def attachments(self):
        """Return a queryset of all attachments, excluding sample test data."""
        return self.file_set.exclude(type=settings.ATT_TYPE.inout_public).order_by(
            "filename"
        )
    ...
    def samples(self) -> QuerySet["File"]:
        return self.file_set.filter(type=settings.ATT_TYPE.inout_public).order_by(
            "filename"
        )
    ...
    def sample_validity(self) -> Mapping[str, int]:
        ...
        sample_testcases = (
            TestCase.objects.filter(
                name__endswith=".pubin",
                upload__task_id=self.id,
                upload__is_official_solution=True,
            )
            .order_by()
            .only("run_status", "points", "max_points", "upload_id", "name")
        )
...
class SetQuerySet(PermissionQuerySetMixin, models.QuerySet):
    class Meta:
        db_table = "ui_core_tasks_setqueryset"
...
class Set(models.Model):
    ...
    description = models.TextField(
        ...
        verbose_name=_("Description"),
        help_text=_(
            "You can use Markdown such as *...* or `...` and LaTeX with $...$.'"
        ),
    )
    public = models.BooleanField(
        default=False,
        verbose_name=_("Public"),
        help_text=_("This set will be visible to all users, even if not logged in."),
    )
    ...
    def tasks_with_links_and_titles(self, pref_lang) -> List[Task]:
        ...
        query = (
            self.links.order_by("sort")
            .select_related("task")
            .annotate(best_title=best_title_subquery(pref_lang, "task_id", Task))
        )
    ...
    class Meta(TypedModelMeta):
        constraints = [
            models.UniqueConstraint(
                fields=("parent", "task"), name="%(app_label)s_%(class)s_unique"
            ),
        ]
...
class FileQuerySet(models.QuerySet):
    def inputs_and_outputs(self) -> "FileQuerySet":
        return self.filter(
            type__in=(settings.ATT_TYPE.inout_public, settings.ATT_TYPE.inout_secret)
        )
    ...
    class Meta:
        db_table = "ui_core_tasks_filequeryset"
...
class File(models.Model):
    ...
    type = models.IntegerField(
        ...
        null=False,
        choices=settings.ATT_TYPE.choices,
        default=None,
    )
    data = models.BinaryField(verbose_name=_("Data"), null=False)
    md5 = models.CharField(
        verbose_name=_("MD5 hash"), max_length=32, null=False, default="deadbeef"
    )  # should not be set manually; computed from data automatically on each save()
    ...
    def save(self, *args, update_fields=None, **kwargs):
        if self.type == settings.ATT_TYPE.inout_public and not (
            self.filename.endswith(".pubin") or self.filename.endswith(".pubout")
        ):
            raise ValueError(
                "A file of type inout_public must have extension .pubin or .pubout; got %s."
                % repr(self.filename)
            )
        if self.type == settings.ATT_TYPE.inout_secret and not (
            self.filename.endswith(".in") or self.filename.endswith(".out")
        ):
            raise ValueError(
                "A file of type inout_secret must have extension .in or .out; got %s."
                % repr(self.filename)
            )
        self.compute_md5()
