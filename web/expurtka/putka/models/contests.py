...
class Contest(models.Model):
    ...
    set = models.ForeignKey(
        ...
        blank=False,
        verbose_name=_("Set"),
        help_text=_("The set with tasks for this contest."),
    )
    ...
    duration_minutes = models.PositiveIntegerField(
        ...
        help_text=_("Duration of the contest in minutes."),
    )
    autostart = models.BooleanField(
        default=False,
        verbose_name=_("Autostart"),
        help_text=_("Start the contest automatically at the specified time."),
    )
    live_results = models.BooleanField(
        verbose_name=_("Live results"),
        help_text=_("Display current scoreboard to contestants during the contest."),
    )
    auditable_results = models.BooleanField(
        default=False,
        verbose_name=_("Auditable results"),
        help_text=_("Show submissions to everyone after the contest."),
    )
    public_testdata = models.BooleanField(
        default=False,
        verbose_name=_("Public test data"),
        help_text=_("Enable downloading test data zips after the contest."),
    )
    scoring_type = models.IntegerField(
        choices=settings.CONTEST_SCORING_TYPES.choices, verbose_name=_("Scoring type")
    )
    freeze_minutes = models.PositiveIntegerField(
        null=True,
        blank=True,
        verbose_name=_("Freeze minutes"),
        help_text=_(
            "How many minutes before the end of the contest the scoreboard should freeze. Scoreboard-default behaviour if empty."
        ),
    )
    ...
    def time_until_str(self):
        ...
        remain = int(
            (self.actual_start + self.duration - timezone.now()).total_seconds()
        )
        if remain < 0:
            return "0:00:00"
    ...
    def state(self) -> int:
        if self.actual_start is None:
            if self.actual_end is None and not self.published:
                ...
                raise DataError(
                    f"Contest object in an invalid state! (actual_start={self.actual_start}, "
    ...
    class Meta(TypedModelMeta):
        constraints = [
            models.UniqueConstraint(
                fields=("contest", "user"), name="%(app_label)s_%(class)s_unique"
            ),
        ]
...
class ContestUpload(models.Model):
    ...
    class Meta(TypedModelMeta):
        constraints = [
            models.UniqueConstraint(
                fields=("contest", "upload"), name="%(app_label)s_%(class)s_unique"
            ),
        ]
...
class ContestGroup(models.Model):
    ...
    scoring_type = models.IntegerField(
        choices=settings.CONTEST_GROUP_SCORING_TYPES.choices,
...
class ContestGroupMembership(models.Model):
    ...
    round_type = models.IntegerField(
        choices=settings.CONTEST_ROUND_TYPES.choices, verbose_name=_("Round type")
    )

    class Meta(TypedModelMeta):
        constraints = [
            models.UniqueConstraint(
                fields=("contest", "group"), name="%(app_label)s_%(class)s_unique"
            ),
        ]
