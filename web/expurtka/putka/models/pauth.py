...
class UserObjectPermission(models.Model):
	user = models.ForeignKey(settings.AUTH_USER_MODEL, related_name='operms', on_delete=models.CASCADE)
	...
	class Meta(TypedModelMeta):
		constraints = [
			models.UniqueConstraint(fields=('user', 'manage', 'content_type', 'object_id'), name='%(app_label)s_%(class)s_unique'),
		]
...
class GroupObjectPermission(models.Model):
	...
	class Meta(TypedModelMeta):
		constraints = [
			models.UniqueConstraint(fields=('group', 'manage', 'content_type', 'object_id'), name='%(app_label)s_%(class)s_unique'),
		]
