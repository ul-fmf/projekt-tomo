...
class UploadBase(models.Model):
	...
	# processing
	status = models.IntegerField(choices=settings.UPLOAD_STATUS.choices, default=settings.UPLOAD_STATUS.waiting)  # Processing stage.
	mgr_status = models.IntegerField(default=-1, null=True, blank=True)  # Manager only, for queuing. UI does not touch.
	preparation_status = models.IntegerField(choices=settings.JAILRUN_STATUS.choices, null=True, blank=True)  # compile job
	preparation_output = models.BinaryField(null=True, blank=True)  # compile job stderr+stdout
	system_output = models.BinaryField(null=True, blank=True)  # testscript stdout+stderr (e.g. stack trace in event of tester failure)
...
class UploadQuerySet(models.QuerySet):
	...
	def with_contests(self) -> 'UploadQuerySet':
		return self.prefetch_related(Prefetch('contests', queryset=Contest.objects.only('title', 'url'), to_attr='contest_list'))
...
class Upload(UploadBase):
	...
	agg_status = models.IntegerField(choices=settings.JAILRUN_STATUS.choices, null=True, blank=True)
	reviewer_comment = models.TextField(null=True, blank=True)  # a manually entered comment, usually for manual grading
	reviewer = models.ForeignKey(settings.AUTH_USER_MODEL, null=True, blank=True, related_name='reviewed', on_delete=models.SET_NULL)
	...
	def public_testcases_with_files(self) -> List['TestCase']:
		...
		return self._augment_testcases_with_files(list(self.testcase_set.filter(name__endswith='.pubin')))
	...
	def _augment_testcases_with_files(self, test_cases: List['TestCase']) -> List['TestCase']:
		test_files = {f.filename: f for f in self.task.file_set.inputs_and_outputs().only('filename', 'task_id')}  # 'task_id' needed for the ORM
		for test_case in test_cases:
			fn_in = test_case.name
			fn_out = INPUT_FILENAME_REGEX.sub(r'\1out', fn_in)
			test_case.data_files = {'input': test_files.get(fn_in), 'output': test_files.get(fn_out)}
		return test_cases
...
class Stat(models.Model):
	run_status = models.IntegerField(choices=settings.JAILRUN_STATUS.choices, null=True, blank=True)
...
class TestCase(Stat):
	name = models.CharField(max_length=100, help_text=_(
		"Input filename (by convention) or another descriptive name"
	))
	upload = models.ForeignKey(Upload, on_delete=models.CASCADE)
	points = models.IntegerField()
	max_points = models.IntegerField()
	user_output = models.BinaryField(null=True, blank=True)
	user_output_line = models.IntegerField(null=True, blank=True, help_text=_(
	...
	official_output_line = models.IntegerField(null=True, blank=True, help_text=_(
	...
	def status(self) -> Optional[int]:
		if self.run_status == settings.JAILRUN_STATUS.OK and self.points != self.max_points:
			return settings.JAILRUN_STATUS.WA
		return self.run_status

	def pretty_score(self) -> SafeString:
		is_public = self.name.endswith('.pubin')
		return mark_safe('<span class="%s">%d/%d</span>' % (
			'fail' if (self.points == 0 and self.max_points > 0) else 'partial' if (self.points < self.max_points) else 'ok',
			0 if is_public else self.points,  # Print zeros to indicate that samples don't count towards the score.
...
class UserTest(Stat, UploadBase):
	...
	def is_compile_error(self):
		return self.preparation_status is not None and self.preparation_status != settings.JAILRUN_STATUS.OK
