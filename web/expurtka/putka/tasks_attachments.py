...
PUBLIC_TYPES = (settings.ATT_TYPE.image, settings.ATT_TYPE.inout_public, settings.ATT_TYPE.generic_public)
...
def guess_att_type(filename):
	...
	return TYPE_ASSOCIATIONS.get(os.path.splitext(filename)[1], settings.ATT_TYPE.generic_secret)
...
def compute_preview(att, preview_mode):
	...
	"""
	data = bytes(att.data)
	mime_type = mimetypes.guess_type(att.filename, strict=False)[0] or 'application/octet-stream'
	...
	if preview_mode == 'plain':
		if att.type == settings.ATT_TYPE.image:
			...
			if len(data) > PREVIEW_SIZE:
				data = data[:PREVIEW_SIZE // 2] + b'\n[ ... truncated ... ]\n' + data[-PREVIEW_SIZE // 2:] + b'\n[ NOTE: truncated in the middle ]'
			html = format_html("<pre>{}</pre>", data.decode('utf-8', 'replace'))
			return HttpResponse(html, content_type='text/html')  # text/plain forces download if weird characters are present in the response
...
def hex_dump(data: bytes, line_len=8):
	...
	for start in range(0, len(data), line_len):
		...
		hex_bytes = ''.join('{:02x}{}'.format(byte_slice[i], ' | ' if (i + 1) % group_size == 0 and i != line_len - 1 else ' ') for i in range(s))
		align_chars = '   ' * (line_len - s) + '  ' * ((line_len - s) // group_size) + ':: '
		text = []
		for i in range(s):
