...
def _get_best_content(content_list: Iterable[Content], preferred_lang: str) -> Optional[Content]:
...
class ContentManagementMixin:
	...
	def fetch_contents(self, query=None, refresh_from_db=False):
		...
		"""
		if hasattr(self, 'contents') and self.contents and not refresh_from_db:  # Data has already been prefetched.
			return
		if query is None:
	...
	def _find_best_content(self, pref_lang):
		...
		assert best_content is not None, f'No contents found for object [{self.pk}]{self}'
...
def best_title_subquery(pref_lang: str, id_field: str, model):
	...
	"""
	return Subquery(
		Content.objects.filter(object_id=OuterRef(id_field), content_type=ContentType.objects.get_for_model(model))
		.lang_pref_order(pref_lang)[:1].values('title')
	)


class ContentQuerySetMixin:
	def with_best_title(self, pref_lang):
		return self.annotate(best_title=best_title_subquery(pref_lang, 'id', self.model))
