...
def superuser_required(view_func=None, redirect_field_name=REDIRECT_FIELD_NAME, login_url=settings.LOGIN_URL):
	...
	actual_decorator = user_passes_test(test, login_url=login_url, redirect_field_name=redirect_field_name)
	if view_func:
		return actual_decorator(view_func)
	return actual_decorator


def staff_required(view_func=None, redirect_field_name=REDIRECT_FIELD_NAME, login_url=settings.LOGIN_URL):
...
class PermissionQuerySetMixin(Generic[M]):
	def only_if_explicit_perm(self: QuerySetProtocol[M], user_id: int, manage: bool) -> QuerySet[M]:
		ct = ContentType.objects.get_for_model(self.model)
		manage_q = {'manage': manage} if manage else {}
		return self.filter(
			Exists(UserObjectPermission.objects.filter(
				user_id=user_id, object_id=OuterRef('id'), content_type=ct, **manage_q,
			))
			| Exists(GroupObjectPermission.objects.filter(
				group__users=user_id, object_id=OuterRef('id'), content_type=ct, **manage_q,
			))
		)
