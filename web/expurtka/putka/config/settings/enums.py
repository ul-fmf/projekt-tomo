...
class CONTEST_GROUP_SCORING_TYPES_(IntegerChoices):
    ...
    )  # Makes the contest group purely administrative, with no aggregate scoreboard.
    UPM = 2, _("UPM")


CONTEST_GROUP_SCORING_TYPES: Type[
    CONTEST_GROUP_SCORING_TYPES_
] = CONTEST_GROUP_SCORING_TYPES_
...
assert set(ALERT_TYPES) == set(ALERT_SUBCLASS_MAP), "{} vs. {}".format(
    set(ALERT_TYPES), set(ALERT_SUBCLASS_MAP)
)
...
class SAMPLE_VALIDITY_(IntegerChoices):
    ...
    mixed = 3, _(
        "Some official solutions agree and some disagree with this sample case."
    )
    undetermined = 4, _(
        "Some official solutions were not evaluated on this sample case."
    )
